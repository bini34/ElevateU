...
Authentication pages share `components/auth/AuthPage`, `AuthForm` and `useAuthForm`, with pure validation/safe error translation in `lib/auth-form.ts`. AuthContext exposes initializing/authenticated/guest/unavailable verification states; login and local-session clearing invalidate older verification callbacks. Public auth-entry redirects wait for verified identity and always target `/`; reset links remain accessible to signed-in users. A protected 401 still clears the expired token, but does not redirect away from public auth pages and discard recovery query state. See [the authentication UI conventions](DESIGN_SYSTEM.md#authentication-experience-day-7).
