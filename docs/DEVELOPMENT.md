...
The development server exposes /design-system for visual and keyboard inspection. Production builds return 404 for that route. The [design-system guide](DESIGN_SYSTEM.md) documents tokens, component APIs, theme scope, browser test setup and the verification record. Run the dev server and production build sequentially because they share .next. Playwright is optional tooling installed outside the app; project dependencies and lockfiles are unchanged. Existing HTTP/WebSocket test commands remain valid.
